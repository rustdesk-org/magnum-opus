...
fn link_pkg_config(name: &str) -> Vec<PathBuf> {
    ...
    #[cfg(all(target_os = "windows", feature = "msys2-pkg-config"))]
    let expect_msg = format!("unable to find '{name}' development headers with pkg-config (feature msys2-pkg-config is enabled).\
    make sure you have installed 'pkgconf' and '{name}' under your corresponding msys2 environment.");
